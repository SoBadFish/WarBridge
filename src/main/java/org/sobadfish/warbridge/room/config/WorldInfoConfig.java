...
public class WorldInfoConfig {
    ...
    public static boolean initWorld(String roomName,String levelName){
        //插件的地图
        File nameFile = new File(WarBridgeMain.getWarBridgeMain().getDataFolder()+File.separator+"rooms"+File.separator+roomName);
        //主世界地图
        ...
        if(world.exists() && world.isDirectory()){
            if(toPathWorld(roomName, levelName)){
                ...
                WarBridgeMain.sendMessageToConsole("&a地图 &e"+levelName+" &a初始化完成");
            }else{
                WarBridgeMain.sendMessageToConsole("&c地图 &e"+levelName+" &c初始化失败,无法完成房间的加载");
                return false;
            }
        }
        if(!world.exists()){
            if(toBackUpWorld(roomName, levelName)){
                WarBridgeMain.sendMessageToConsole("&a地图 &e"+levelName+" &a备份完成");
            }else{
                WarBridgeMain.sendMessageToConsole("&c地图 &e"+levelName+" &c备份失败,无法完成房间的加载");
                return false;
            }
        }
