...
public class GameRoomCreater {
    ...
    private boolean createScorePos(){
        ...
        if(teamScore.size() == roomConfig.getTeamCfg().size()){
            ...
            creater.sendForceMessage("&2得分点设置完成");
            ArrayList<TeamInfoConfig> teamInfoConfigs = new ArrayList<>();
            for(String teamName : team.keySet()){
                TeamInfoConfig teamInfoConfig = new TeamInfoConfig(roomConfig.teamCfg.get(teamName),team.get(teamName),teamScore.get(teamName));
