...
public class BaseDataWriterGetterManager<T>{
    ...
    public static <T> BaseDataWriterGetterManager<?> asFile(File file,String fileName, Class<T> tClass,Class<? extends BaseDataWriterGetterManager<?>> baseClass){
        ...
        try {
            if(!file.exists()){
                WarBridgeMain.getWarBridgeMain().saveResource(fileName,false);
            }
            ...
            WarBridgeMain.sendMessageToConsole("&c无法读取 "+file.getName()+" 配置文件");
            e.printStackTrace();
            ...
            if(reader !=null){
                try {
                    ...
                    WarBridgeMain.sendMessageToConsole("&c"+ Throwables.getStackTraceAsString(e));
                }
            }
        }
        return null;
        ...
        if(!file.exists()){
            try {
                if(!file.createNewFile()){
                    WarBridgeMain.sendMessageToConsole("&c创建文件失败");
                }
            } catch (IOException e) {
                WarBridgeMain.sendMessageToConsole("未知错误 无法保存玩家数据");
            }
        }
        try {
